...
(defun helm-ag--find-file-action (candidate find-func this-file &optional persistent)
  ...
  (let* ((file-line (helm-grep-split-line candidate))
    ...
    (tabbar-window-add-tab (tabbar-current-tabset) (current-buffer) t)
    (goto-char (point-min))
    (when line
...
(defun helm-ag--action-find-file (candidate)
  (let ((candidates (helm-marked-candidates)))
    (if (>= (length candidates) 1)
        (cl-loop for candidate-file in candidates
                 do
                     (helm-ag--find-file-action candidate-file 'find-file (helm-ag--search-this-file-p)))
